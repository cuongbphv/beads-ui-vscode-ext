---
name: bead-loop
description: Dùng khi user gõ /bead-loop, hoặc yêu cầu lặp qua board beads làm lần lượt từng việc tự làm được. Mỗi vòng nhận đúng MỘT bead và giao bằng bằng chứng.
disable-model-invocation: true
---

# bead-loop

Bạn đang chạy một vòng của `/bead-loop`. **Tham số = phần text user gõ sau lệnh**
(`[--include-partial] [--dry-run]`).
...
Thay `<args>` bằng đúng chuỗi tham số của lượt này. Bốn chi tiết đã đo, mỗi cái đều
làm script sai **lặng** nếu bỏ:
...
- Đang trong chế độ lặp tự động của harness: dừng vòng lặp theo cơ chế của harness,
  không schedule vòng mới.
...
Làm theo skill `bead-take` (xem `.cursor/skills/bead-take/SKILL.md`) với id vừa
chọn. Nó đã mang toàn bộ quy trình: đọc kỹ `bd show` (điều kiện đóng nằm trong
NOTES, note RE-MEASURE mới nhất đáng tin hơn description), worktree riêng nhánh từ
`<BASE>` (nhánh tích hợp của dự án), TDD/systematic-debugging theo loại bead, đo
trước khi sửa, `git commit --only <path>`, đóng bằng bằng chứng.
...
Không todo tool, không markdown checkbox, không mở rộng scope bead đang làm.
