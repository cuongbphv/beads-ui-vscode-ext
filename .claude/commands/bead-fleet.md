---
description: Fleet song song nhiều bead auto-ok tới khi hết việc — mỗi bead một worktree, rebase + ff-only về nhánh tích hợp, dọn worktree
argument-hint: "[--batch N] [--include-partial] [--unattended]"
---

Bạn chạy **fleet** `/bead-fleet`. Tham số: **$ARGUMENTS**
...
- **Mọi ghi lên beads** (`--claim`, `close`, `create`, `dep`, `--append-notes`) do
  **BẠN** chạy ở cây chính. Agent chỉ ĐỌC. `bd list`/`bd show` từ worktree đã đo là
  chạy đúng (`.beads/metadata.json` được track). [Unverified] hai `bd` ghi đồng thời
  có an toàn không → đừng thử.
...
Extract khối bash §1 của `.claude/commands/bead-loop.md` bằng `awk` rồi chạy — đừng gõ
lại (nó là nguồn duy nhất của luật nhãn + lọc blocker). Từ hàng `auto-ok` sẵn sàng,
lọc tiếp:
...
```
Công cụ worktree của harness (EnterWorktree…) chỉ dùng khi chắc nó nhánh từ đúng
`<BASE>` — nhiều harness mặc định nhánh từ `origin/main`, thiếu commit nhánh tích hợp.
...
- Chạy **full gate của repo**: lint + typecheck + full test suite — lệnh lấy từ
  CLAUDE.md / cấu hình CI / script trong `package.json`/`pyproject`/`Makefile`,
  không tự bịa.
...
- Đọc `CLAUDE.md` / doc quy ước của repo (nếu có).
...
- Bug → skill `superpowers:systematic-debugging`. Feature/refactor → `test-driven-development`.
...
NỢ DO THAY ĐỔI CỦA BẠN SINH RA → fix sạch trong worktree này, cùng commit:
  ...
  (registry, audit log, migration checklist… — đọc CLAUDE.md) làm đủ.
NỢ CÓ TRƯỚC, không liên quan → báo orchestrator `bd create`. KHÔNG mở rộng scope,
  KHÔNG TodoWrite, KHÔNG markdown checkbox.
