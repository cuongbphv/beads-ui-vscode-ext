...
export function WorkerList({ snapshot }: { snapshot: FleetSnapshot }): ReactNode {
      ...
      {snapshot.orchestrators.map((orchestrator) => {
        ...
        return (
          <section key={orchestrator.sessionId} className="border-border rounded-md border">
            <header className="border-border text-fg-muted flex items-center gap-2 border-b px-3 py-2 text-xs">
            ...
            <ul className="divide-border divide-y">
              {workers.map((worker) => {
                ...
                return (
                  <li key={worker.agentId} className="flex flex-col gap-1 px-3 py-2">
